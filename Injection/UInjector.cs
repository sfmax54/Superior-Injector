...
namespace ManualMapInjection.Injection
{
    internal class UInjector
    {
        ...
        public IntPtr ManualMapInject(byte[] buffer)
        {
        ...
        public IntPtr ManualMapInject(string file)
        {
            return ManualMapInject(File.ReadAllBytes(file));
        }

        public void LoadLibraryInject(string dllName)
        {
            IntPtr procHandle = Imports.OpenProcess(_process, Imports.ProcessAccessFlags.All);
            ...
            IntPtr allocMemAddress = Imports.VirtualAllocEx(procHandle, UIntPtr.Zero, (IntPtr)((dllName.Length + 1) * Marshal.SizeOf(typeof(char))), Imports.AllocationType.Commit | Imports.AllocationType.Reserve, Imports.MemoryProtection.ExecuteReadWrite);

            // writing the name of the dll there
            UIntPtr bytesWritten;
            Imports.WriteProcessMemory(procHandle, allocMemAddress, Encoding.ASCII.GetBytes(dllName), Encoding.ASCII.GetBytes(dllName).Length, out bytesWritten);

            // creating a thread that will call LoadLibraryA with allocMemAddress as argument
            Imports.CreateRemoteThread(procHandle, IntPtr.Zero, 0, loadLibraryAddr, allocMemAddress, 0, IntPtr.Zero);
        }

        #endregion

        public UInjector(Process p)
        {
            _process = p;
        }
    }
